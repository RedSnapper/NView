...
trait Form
{
     ...
     * Used to identify whether or not the formlet has committed. Will be true only during a post, when the inputs are
     * all valid, and the commit has occured.
     *
     * @return bool
     */
    ...
    { //ident includes userid and is used as a SCRF Token
        $ident = '';
        ...
        if (isset($this->fields[$input])) {
            ...
            if (is_array($this->dflds[$input])) {
                foreach ($this->dflds[$input] as $val) {
                    $this->view->set("//h:input[@type='radio'][@value='" . $val . "'][@name='$input']/@checked",
                      "checked");
                }
            } else {
    ...
    protected function vsetopts($input = '_unused', $qvp = 'select 0 as prompt,0 as value', $v = null)
    {
        ...
        if (isset($err) && (mb_strlen($err) > 0)) {
            ...
            if (isset($this->fields[$input])) {
                foreach ($this->fields[$input] as $val) {
                    $this->view->set("//h:select[@name='$input']/h:option[@value='" . $val . "']/@selected",
                      "selected");
                }
            } else {
                if (is_array($this->dflds[$input])) {
        ...
        if ($us) {
            ...
            foreach ($fms as $fm) {
                ...
                if ($fm->hasFunc()) {
                    ...
                    if (!is_null($func)) {
                        if ($debug) {
                            print("<p>{$fm->sig()}:Use only one formlet with a func in formlets()</p>");
                        }
                    } else {
                        $func = $fm->func();
                    }
            ...
            if (!is_null($func)) { //do function other than save.
                ...
                foreach ($fms as $fm) {
                    ...
                    if (is_null($sfn) || ($spv != "fm")) {
                        ...
                        $valid = $valid && $fmval;
                        if ($debug) {
                            $vtxt = $fmval ? "true" : "false";
                            print("<p>{$fm->sig()}: Validate returned $vtxt</p>");
                        }
                    }
                }
                ...
                if ($valid) {
                    ...
                    foreach ($fms as $fm) {
                        ...
                        if (!$fm->success()) {
                            ...
                            if ($debug) {
                                print("<p>{$fm->sig()}: preset to successful</p>");
                            }
                        }
                        $success &= $fm->success();
                    }
    ...
    public function forminfo()
    {
        ...
        if (isset($_POST[static::sig()])) {
            ...
            if (in_array($this->ident(), $_POST[static::sig()])) {
                ...
                print("and the post failed to match because (post)" . print_r($_POST[static::sig()],
                    true) . "!=(ident)" . $this->ident() . "<br />");
            }
        } else {
    ...
    public function form($show = true)
    {
        ...
        if ($this->isMe()) {
            ...
            $this->setfrompost();
            if (isset($this->fields['_fn'][0]) && (mb_strlen($this->fields['_fn'][0]) > 0) && (strcmp($this->fields['_fn'][0],
                  "save") !== 0)) {
                if ($this->debug) {
    ...
    public function reveal()
    {
        if ($this->show || !($this->committed)) { //don't repop if !show and com.
            ...
            $this->view->set("//*[@data-xp='_ident']/@name",
              static::sig() . "[]"); //we have to force this to be an array.
    ...
    {
        if (isset($this->fields[$name][0])) {
            if ((mb_strlen($this->fields[$name][0]) > 0) && (!is_string($this->fields[$name][0]) || preg_match('/\s/',
                  $this->fields[$name][0]))) {
                $this->valid = false;
                $this->seterr($name, $msg);
            }
    ...
    {
        if (isset($this->fields[$name][0])) {
            if (!filter_var($this->fields[$name][0], FILTER_VALIDATE_REGEXP,
