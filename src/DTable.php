...
class DTable
{
    ...
    static $defaults = array(
      ...
      */
      ...
      'request'      => null // Use your own request
    );
    ...
    {
        $this->envGet = Settings::create(EnvGet::class);
        $this->envPost = Settings::create(EnvPost::class);
        ...
        $query = "select {$this->fields} from {$this->tables} {$restriction} {$groupby} limit 1";

        $md = $this->sql_exec($query)->result_metadata();
    ...
    /**
     * Get Dtable from serialized inputs
     *
     * @param string $data
     * @return DTable
     */
    ...
    private function sql_exec($sql = null, $bindings = null)
    {
        ...
        if ($stmt) {
            // Bind parameters
            $length = is_null($bindings) ? 0 : count($bindings);
            if ($length > 0) {
                $a_params = array();
                for ($i = 0; $i < $length; $i++) {
                    $a_params[] = &$bindings[$i];
    ...
    private function getRequest(): DTableRequest
    {
        ...
        return new DTableRequest($this->getRequestFromEnvironment());
    }
